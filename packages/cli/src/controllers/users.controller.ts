...
export class UsersController {
	 ...
	 */
	@Post('/createuser')
	async createSflowUser(req: UserRequest.SflowSignUp, res: Response) {
		const { apikey, sflowOrg, sflowUid, sflowEmail, firstName, lastName, password } = req.body;
		if (!apikey) {
			this.logger.debug('Request failed, API Key is required to authorize Sflow request', {
				payload: req.body,
			});
		...
		if (!sflowUid) {
			this.logger.debug('Request failed because of missing Sflow userId', { payload: req.body });
			throw new BadRequestError('Sflow userId is missing');
		}
		if (!sflowOrg) {
			this.logger.debug('Request failed because of missing Sflow orgId', { payload: req.body });
			throw new BadRequestError('Sflow orgId is missing');
		}
		if (!sflowEmail || !firstName || !lastName || !password) {
			this.logger.debug('Request failed because of missing properties in payload', {
				payload: req.body,
			});
			throw new BadRequestError('Invalid payload');
		}
		if (!validator.isEmail(sflowEmail)) {
			this.logger.debug('Invalid email in payload', { invalidEmail: sflowEmail });
			throw new BadRequestError(`Invalid email address: ${sflowEmail}`);
		}
		...
		if (!role) {
			this.logger.error('No global member role was found in database');
			throw new InternalServerError('Members role not found in database - inconsistent state');
		}

		const user = await this.userRepository.findOneBy({ resetPasswordToken: sflowUid });
		if (user) {
			this.logger.debug('Sflow userId already exists', { payload: req.body });
			// throw new ResponseError('Sflow userId already exists', undefined, 400);
			const uid = user.id;
			return { uid };
		}
		...
		try {
			await this.userRepository.manager.transaction(async (transactionManager) => {
				...
				void this.internalHooks.onUserSignup(savedUser, {
					user_type: 'email',
		...
		this.logger.info('Created user shell successfully', { userId: uid });
		this.logger.verbose('1 user shell created', { userShells: createUsers });
		return { uid };
	}
